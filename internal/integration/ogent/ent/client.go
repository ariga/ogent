...
// A call to `Intercept(f, g, h)` equals to `alltypes.Intercept(f(g(h())))`.
func (c *AllTypesClient) Intercept(interceptors ...Interceptor) {
	c.inters.AllTypes = append(c.inters.AllTypes, interceptors...)
}
...
func (c *AllTypesClient) MapCreateBulk(slice any, setFunc func(*AllTypesCreate, int)) *AllTypesCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AllTypesCreateBulk{err: fmt.Errorf("calling to AllTypesClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AllTypesCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *AllTypesClient) Query() *AllTypesQuery {
	return &AllTypesQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAllTypes},
...
func (c *AllTypesClient) Interceptors() []Interceptor {
	return c.inters.AllTypes
}

func (c *AllTypesClient) mutate(ctx context.Context, m *AllTypesMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AllTypesCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AllTypesUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AllTypesUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AllTypesDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AllTypes mutation op: %q", m.Op())
	}
}
...
// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *HatClient) MapCreateBulk(slice any, setFunc func(*HatCreate, int)) *HatCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &HatCreateBulk{err: fmt.Errorf("calling to HatClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*HatCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
